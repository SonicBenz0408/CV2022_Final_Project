...
class AnchorPyramidNet(nn.Module):
    ...
    def __init__(self, anchor_num=24):
        ...
        self.regression = nn.Sequential(
            nn.Linear(16 * 12 * 12 + 32 * 6 * 6 + 128, 24 * 2 * 68)
        )

        self.confidence = nn.Sequential(
            nn.Linear(16 * 12 * 12 + 32 * 6 * 6 + 128, 24),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, input):
        ...
        regression_output = torch.reshape(regression_output, (regression_output.shape[0], 24, 68, 2))
